...
let list = []
let formatList = ["YmdHis", "Y年m月d日, h时i分s秒", "Y-m-d h:i:s", "d F Y h:i:s"]
let moreFormatList = ["Ymd", "Y年m月d日", "Y-m-d", "d F Y", "h:i:s", "h:i"]
let time
let quickAddApi
async function getTime(params) {
    ({ quickAddApi } = params)
    return nomalFormat(new Date())

}

async function nomalFormat(data){
    for (let index = 0; index < formatList.length; index++) {
        list[index] = data.format(formatList[index])
    }
    list.push("More...")
    time = await quickAddApi.suggester(list, list)
    if (time == "More...") {
        list = []
        return moreFormat(data)
    }
    return time
}

async function moreFormat(data){
    for (let index = 0; index < moreFormatList.length; index++) {
        list[index] = data.format(moreFormatList[index])
    }
    list.push("Back")
    time = await quickAddApi.suggester(list, list);
    if (time == "Back") {
        list = []
        return nomalFormat(data)
    }
    return time
}
